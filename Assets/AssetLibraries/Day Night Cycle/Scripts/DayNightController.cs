...
public class DayNightController : MonoBehaviour {
    ...
    [NonSerialized] public float currentTime = 0.0f;
	...
	void StarSphere() {
		...
		int chosenOne = UnityEngine.Random.Range (0, starSpheres.Length);
		...
		if (starSpheres [chosenOne] && twinkleCounter <= 0.0f && (currentTime >= 18.0f || currentTime <= 5.5f) && starSpheres [chosenOne].GetComponent<Renderer>()) {
			...
			starSpheres [chosenOne].GetComponent<Renderer>().material.color = new Color (currentColor.r,currentColor.g,currentColor.b,UnityEngine.Random.Range(0.01f,0.5f));
		}
		//Reset counter
